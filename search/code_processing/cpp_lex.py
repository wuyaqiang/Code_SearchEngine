...
_key=("strlen","alignas","alignof","asm","auto","bool","break","case","catch","char","char16_t","char32_t",
      "class","const","constexpr","const_cast","continue","decltype","default","delete","do","double","dynamic_cast",
        ...
        if match or i in _bounds or i in _mathsign:
            ...
            if word:
                ...
                if word[0] in string.digits:
                    ...
                    _lexfile=_lexfile+str(num)+'\t'+str(_line)+'\t'+word+'\t'+newsign[sign-1]+'\n'
                else:
                    if word in _mathsign:
                        ...
                        sign=1
                        num=num+1
                        # _lexfile+str(num)+'\t'+str(_line)+'\t'+word+'\t'+str(sign)+'\n'
                        _lexfile=_lexfile+str(num)+'\t'+str(_line)+'\t'+word+'\t'+newsign[sign-1]+'\n'
            ...
            if i in _bounds:
                ...
                _lexfile=_lexfile+str(num)+'\t'+str(_line)+'\t'+i+'\t'+newsign[sign-1]+'\n'
